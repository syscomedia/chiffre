    ...
    return (
        <div className="min-h-screen bg-[#0a0a0a] text-white font-sans flex overflow-hidden">
            ...
            <main className="flex-1 flex flex-col relative">
                ...
                {/* Header */}
                <header className="px-8 py-6 flex items-center justify-between border-b border-white/5 backdrop-blur-md bg-black/20 z-10">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-2xl bg-gradient-to-tr from-[#4a3426] to-[#c69f6e] p-[1px]">
                            <div className="w-full h-full bg-[#0a0a0a] rounded-[15px] flex items-center justify-center">
                                <Bot size={24} className="text-[#c69f6e]" />
                            </div>
                        </div>
                        <div>
                            <h1 className="text-xl font-black tracking-tight uppercase">AI Business Master</h1>
                ...
                <div ref={scrollRef} className="flex-1 overflow-y-auto p-8 space-y-8 pb-32">
                    {messages.length === 0 ? (
                        ...
                        messages.map((msg, i) => (
                            ...
                            >
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${msg.role === 'user' ? 'bg-[#c69f6e] text-white' : 'bg-white/10 text-[#c69f6e]'}`}>
                                    {msg.role === 'user' ? <User size={20} /> : <Bot size={20} />}
                                </div>
                                <div className={`max-w-[80%] p-6 rounded-[2rem] text-sm leading-relaxed ${msg.role === 'user' ? 'bg-[#c69f6e]/10 border border-[#c69f6e]/20 text-[#c69f6e]' : 'bg-white/[0.03] border border-white/10 text-white/90 shadow-xl'}`}>
                                    {msg.content}
                                </div>
                            </motion.div>
                        ))
                ...
                {/* Input Area */}
                <div className="absolute bottom-0 left-0 right-0 p-8 bg-gradient-to-t from-[#0a0a0a] via-[#0a0a0a]/95 to-transparent pt-12">
                    <div className="max-w-4xl mx-auto flex items-center gap-4">
                        <div className="flex-1 relative">
                            <input
                                ...
                                className="w-full h-16 bg-white/[0.03] border border-white/10 rounded-3xl pl-6 pr-16 text-sm font-medium outline-none focus:border-[#c69f6e]/40 focus:bg-white/[0.05] transition-all"
                            />
                            <button
                                onClick={() => input.trim() && sendMessageToAI(input)}
                                disabled={!input.trim() || isTyping}
                                className="absolute right-2 top-1/2 -translate-y-1/2 p-3 bg-[#c69f6e] hover:bg-[#c69f6e]/80 rounded-2xl text-white transition-all disabled:opacity-30"
                            >
                                <Send size={20} />
                            </button>
                        </div>
                        ...
                        <button
                            ...
                            className={`h-16 px-8 rounded-3xl flex items-center gap-3 font-black text-[10px] uppercase tracking-[0.2em] transition-all shadow-2xl ${isCalling ? 'bg-red-500 text-white' : 'bg-white text-black hover:scale-105 active:scale-95'}`}
                        >
                        ...
                        >
                            ...
                            <div className="flex flex-col items-center gap-8 text-center px-4 w-full max-w-lg">
                                ...
                                <div className="relative">
                                    ...
                                    />
                                    <div className="relative w-40 h-40 bg-gradient-to-tr from-[#4a3426] to-[#c69f6e] rounded-full p-[2px]">
                                        <div className="w-full h-full bg-black rounded-full flex items-center justify-center">
                                            {isSpeaking ? (
                                                <div className="flex items-center gap-1">
                                                    {[1, 2, 3, 4, 5].map(i => (
                                                        <motion.div
                                                            ...
                                                            className="w-1.5 bg-[#c69f6e] rounded-full"
                                                        />
                                                    ))}
                                                </div>
                                            ) : isRecording ? (
                                                ...
                                                <Bot size={64} className="text-[#c69f6e]" />
                                            )}
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="flex items-center gap-4">
                                    <button
                                        ...
                                        className={`w-14 h-14 rounded-full flex items-center justify-center transition-all ${voiceEnabled ? 'bg-white/10 text-white' : 'bg-red-500/20 text-red-400'}`}
                                    >
                                    ...
                                    <button
                                        ...
                                        className={`w-20 h-20 rounded-full flex items-center justify-center transition-all ${isRecording ? 'bg-red-500 shadow-[0_0_30px_rgba(239,68,68,0.5)]' : 'bg-white/10 hover:bg-white/20 disabled:opacity-40'}`}
                                    >
                                        <Mic size={32} />
                                    </button>
                                    ...
                                    <button
                                        ...
                                        className={`w-14 h-14 rounded-full flex items-center justify-center transition-all ${showKeyboard ? 'bg-[#c69f6e] text-white' : 'bg-white/10 text-white'}`}
                                    >
                                        <Keyboard size={24} />
                                    </button>
                                    ...
                                    <button
                                        onClick={stopCall}
                                        className="w-20 h-20 bg-red-500 rounded-full flex items-center justify-center shadow-[0_0_30px_rgba(239,68,68,0.4)] hover:scale-105 transition-all"
                                    >
                                        <PhoneOff size={32} />
                                    </button>
                                </div>
